...
import 'table_operations_test.mocks.dart';
...
void main() {
  ...
  group('TableOperations', () {
    ...
    test('createTable creates a new table', () async {
      const databaseId = 1;
      ...
      expect(table.databaseId, databaseId);
      expect(table.order, 1);
      expect(table.dataSync, isNull);
    });

    test('createTable throws when user has no access', () async {
      const databaseId = 1;
      when(mockClient.post(
        Uri.parse('http://localhost/api/database/tables/database/$databaseId/'),
        headers: anyNamed('headers'),
      ...
      expect(
        () => client.createTable(databaseId, name: 'New Table'),
        throwsA(isA<BaserowException>()),
      );
    });
  });
