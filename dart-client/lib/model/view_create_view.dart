...
class ViewCreateView {
  /// Returns a new [ViewCreateView] instance.
  ViewCreateView({
    required this.name,
    required this.type,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ViewCreateView &&
    other.name == name &&
    other.type == type &&
  ...
  @override
  String toString() => 'ViewCreateView[name=$name, type=$type, ownershipType=$ownershipType, filterType=$filterType, filtersDisabled=$filtersDisabled, rowIdentifierType=$rowIdentifierType, rowHeightSize=$rowHeightSize, public=$public, slug=$slug, cardCoverImageField=$cardCoverImageField, title=$title, description=$description, mode=$mode, coverImage=$coverImage, logoImage=$logoImage, submitText=$submitText, submitAction=$submitAction, submitActionMessage=$submitActionMessage, submitActionRedirectUrl=$submitActionRedirectUrl, receiveNotificationOnSubmit=$receiveNotificationOnSubmit, singleSelectField=$singleSelectField, dateField=$dateField, icalFeedUrl=$icalFeedUrl, icalPublic=$icalPublic, startDateField=$startDateField, endDateField=$endDateField, timescale=$timescale]';
  ...
  /// Returns a new [ViewCreateView] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ViewCreateView? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ViewCreateView[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ViewCreateView[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ViewCreateView> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ViewCreateView>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ViewCreateView.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ViewCreateView> mapFromJson(dynamic json) {
    final map = <String, ViewCreateView>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ViewCreateView.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ViewCreateView-objects as value to a dart map
  static Map<String, List<ViewCreateView>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ViewCreateView>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ViewCreateView.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
