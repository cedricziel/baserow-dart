...
class LocalBaserowAggregateRowsDataSource {
  /// Returns a new [LocalBaserowAggregateRowsDataSource] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LocalBaserowAggregateRowsDataSource &&
    other.id == id &&
  ...
  @override
  String toString() => 'LocalBaserowAggregateRowsDataSource[id=$id, integrationId=$integrationId, type=$type, schema=$schema, contextData=$contextData, contextDataSchema=$contextDataSchema, name=$name, pageId=$pageId, order=$order, tableId=$tableId, viewId=$viewId, filters=$filters, filterType=$filterType, searchQuery=$searchQuery, fieldId=$fieldId, aggregationType=$aggregationType]';
  ...
  /// Returns a new [LocalBaserowAggregateRowsDataSource] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LocalBaserowAggregateRowsDataSource? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LocalBaserowAggregateRowsDataSource[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LocalBaserowAggregateRowsDataSource[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LocalBaserowAggregateRowsDataSource(
  ...
  static List<LocalBaserowAggregateRowsDataSource> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LocalBaserowAggregateRowsDataSource>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LocalBaserowAggregateRowsDataSource.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LocalBaserowAggregateRowsDataSource> mapFromJson(dynamic json) {
    final map = <String, LocalBaserowAggregateRowsDataSource>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LocalBaserowAggregateRowsDataSource.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LocalBaserowAggregateRowsDataSource-objects as value to a dart map
  static Map<String, List<LocalBaserowAggregateRowsDataSource>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LocalBaserowAggregateRowsDataSource>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LocalBaserowAggregateRowsDataSource.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
