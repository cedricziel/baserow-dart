...
class LocalBaserowGetRowService {
  /// Returns a new [LocalBaserowGetRowService] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LocalBaserowGetRowService &&
    other.id == id &&
  ...
  @override
  String toString() => 'LocalBaserowGetRowService[id=$id, integrationId=$integrationId, type=$type, schema=$schema, contextData=$contextData, contextDataSchema=$contextDataSchema, tableId=$tableId, viewId=$viewId, filters=$filters, filterType=$filterType, searchQuery=$searchQuery, rowId=$rowId]';
  ...
  /// Returns a new [LocalBaserowGetRowService] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LocalBaserowGetRowService? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LocalBaserowGetRowService[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LocalBaserowGetRowService[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LocalBaserowGetRowService(
  ...
  static List<LocalBaserowGetRowService> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LocalBaserowGetRowService>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LocalBaserowGetRowService.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LocalBaserowGetRowService> mapFromJson(dynamic json) {
    final map = <String, LocalBaserowGetRowService>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LocalBaserowGetRowService.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LocalBaserowGetRowService-objects as value to a dart map
  static Map<String, List<LocalBaserowGetRowService>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LocalBaserowGetRowService>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LocalBaserowGetRowService.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
