...
class LocalBaserowGetRowDataSource {
  /// Returns a new [LocalBaserowGetRowDataSource] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LocalBaserowGetRowDataSource &&
    other.id == id &&
  ...
  @override
  String toString() => 'LocalBaserowGetRowDataSource[id=$id, integrationId=$integrationId, type=$type, schema=$schema, contextData=$contextData, contextDataSchema=$contextDataSchema, name=$name, pageId=$pageId, order=$order, tableId=$tableId, viewId=$viewId, filters=$filters, filterType=$filterType, searchQuery=$searchQuery, rowId=$rowId]';
  ...
  /// Returns a new [LocalBaserowGetRowDataSource] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LocalBaserowGetRowDataSource? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LocalBaserowGetRowDataSource[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LocalBaserowGetRowDataSource[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LocalBaserowGetRowDataSource(
  ...
  static List<LocalBaserowGetRowDataSource> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LocalBaserowGetRowDataSource>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LocalBaserowGetRowDataSource.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LocalBaserowGetRowDataSource> mapFromJson(dynamic json) {
    final map = <String, LocalBaserowGetRowDataSource>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LocalBaserowGetRowDataSource.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LocalBaserowGetRowDataSource-objects as value to a dart map
  static Map<String, List<LocalBaserowGetRowDataSource>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LocalBaserowGetRowDataSource>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LocalBaserowGetRowDataSource.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
