...
class AuditLogExportJobCreateJob {
  /// Returns a new [AuditLogExportJobCreateJob] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AuditLogExportJobCreateJob &&
    other.url == url &&
    other.type == type &&
  ...
  @override
  String toString() => 'AuditLogExportJobCreateJob[url=$url, type=$type, exportCharset=$exportCharset, csvColumnSeparator=$csvColumnSeparator, csvFirstRowHeader=$csvFirstRowHeader, filterUserId=$filterUserId, filterWorkspaceId=$filterWorkspaceId, filterActionType=$filterActionType, filterFromTimestamp=$filterFromTimestamp, filterToTimestamp=$filterToTimestamp, excludeColumns=$excludeColumns]';
  ...
  /// Returns a new [AuditLogExportJobCreateJob] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AuditLogExportJobCreateJob? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AuditLogExportJobCreateJob[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AuditLogExportJobCreateJob[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return AuditLogExportJobCreateJob(
  ...
  static List<AuditLogExportJobCreateJob> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AuditLogExportJobCreateJob>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AuditLogExportJobCreateJob.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AuditLogExportJobCreateJob> mapFromJson(dynamic json) {
    final map = <String, AuditLogExportJobCreateJob>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AuditLogExportJobCreateJob.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AuditLogExportJobCreateJob-objects as value to a dart map
  static Map<String, List<AuditLogExportJobCreateJob>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AuditLogExportJobCreateJob>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AuditLogExportJobCreateJob.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
