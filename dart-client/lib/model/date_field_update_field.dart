...
class DateFieldUpdateField {
  /// Returns a new [DateFieldUpdateField] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is DateFieldUpdateField &&
    other.name == name &&
    other.type == type &&
  ...
  @override
  String toString() => 'DateFieldUpdateField[name=$name, type=$type, description=$description, dateFormat=$dateFormat, dateIncludeTime=$dateIncludeTime, dateTimeFormat=$dateTimeFormat, dateShowTzinfo=$dateShowTzinfo, dateForceTimezone=$dateForceTimezone, dateForceTimezoneOffset=$dateForceTimezoneOffset]';
  ...
  /// Returns a new [DateFieldUpdateField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static DateFieldUpdateField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DateFieldUpdateField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DateFieldUpdateField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return DateFieldUpdateField(
  ...
  static List<DateFieldUpdateField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <DateFieldUpdateField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = DateFieldUpdateField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, DateFieldUpdateField> mapFromJson(dynamic json) {
    final map = <String, DateFieldUpdateField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DateFieldUpdateField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of DateFieldUpdateField-objects as value to a dart map
  static Map<String, List<DateFieldUpdateField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DateFieldUpdateField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = DateFieldUpdateField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
