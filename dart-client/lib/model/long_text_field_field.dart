...
class LongTextFieldField {
  /// Returns a new [LongTextFieldField] instance.
  LongTextFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LongTextFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'LongTextFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description, longTextEnableRichText=$longTextEnableRichText]';
  ...
  /// Returns a new [LongTextFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LongTextFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LongTextFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LongTextFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LongTextFieldField(
  ...
  static List<LongTextFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LongTextFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LongTextFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LongTextFieldField> mapFromJson(dynamic json) {
    final map = <String, LongTextFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LongTextFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LongTextFieldField-objects as value to a dart map
  static Map<String, List<LongTextFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LongTextFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LongTextFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
