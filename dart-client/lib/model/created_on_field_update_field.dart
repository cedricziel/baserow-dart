...
class CreatedOnFieldUpdateField {
  /// Returns a new [CreatedOnFieldUpdateField] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CreatedOnFieldUpdateField &&
    other.name == name &&
    other.type == type &&
  ...
  @override
  String toString() => 'CreatedOnFieldUpdateField[name=$name, type=$type, description=$description, dateFormat=$dateFormat, dateIncludeTime=$dateIncludeTime, dateTimeFormat=$dateTimeFormat, dateShowTzinfo=$dateShowTzinfo, dateForceTimezone=$dateForceTimezone, dateForceTimezoneOffset=$dateForceTimezoneOffset]';
  ...
  /// Returns a new [CreatedOnFieldUpdateField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CreatedOnFieldUpdateField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreatedOnFieldUpdateField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreatedOnFieldUpdateField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return CreatedOnFieldUpdateField(
  ...
  static List<CreatedOnFieldUpdateField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CreatedOnFieldUpdateField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CreatedOnFieldUpdateField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CreatedOnFieldUpdateField> mapFromJson(dynamic json) {
    final map = <String, CreatedOnFieldUpdateField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreatedOnFieldUpdateField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CreatedOnFieldUpdateField-objects as value to a dart map
  static Map<String, List<CreatedOnFieldUpdateField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CreatedOnFieldUpdateField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CreatedOnFieldUpdateField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
