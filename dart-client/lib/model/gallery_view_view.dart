...
class GalleryViewView {
  /// Returns a new [GalleryViewView] instance.
  GalleryViewView({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is GalleryViewView &&
    other.id == id &&
  ...
  @override
  String toString() => 'GalleryViewView[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, table=$table, filterType=$filterType, filters=$filters, filterGroups=$filterGroups, sortings=$sortings, groupBys=$groupBys, decorations=$decorations, filtersDisabled=$filtersDisabled, publicViewHasPassword=$publicViewHasPassword, showLogo=$showLogo, ownershipType=$ownershipType, ownedById=$ownedById, cardCoverImageField=$cardCoverImageField, public=$public, slug=$slug]';
  ...
  /// Returns a new [GalleryViewView] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static GalleryViewView? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GalleryViewView[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GalleryViewView[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<GalleryViewView> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <GalleryViewView>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = GalleryViewView.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, GalleryViewView> mapFromJson(dynamic json) {
    final map = <String, GalleryViewView>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GalleryViewView.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of GalleryViewView-objects as value to a dart map
  static Map<String, List<GalleryViewView>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<GalleryViewView>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = GalleryViewView.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
