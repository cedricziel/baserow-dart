...
class PasswordFieldField {
  /// Returns a new [PasswordFieldField] instance.
  PasswordFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PasswordFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'PasswordFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description]';
  ...
  /// Returns a new [PasswordFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PasswordFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PasswordFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PasswordFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return PasswordFieldField(
  ...
  static List<PasswordFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PasswordFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PasswordFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PasswordFieldField> mapFromJson(dynamic json) {
    final map = <String, PasswordFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PasswordFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PasswordFieldField-objects as value to a dart map
  static Map<String, List<PasswordFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PasswordFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PasswordFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
