...
class MultipleCollaboratorsFieldField {
  /// Returns a new [MultipleCollaboratorsFieldField] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is MultipleCollaboratorsFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'MultipleCollaboratorsFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description, notifyUserWhenAdded=$notifyUserWhenAdded]';
  ...
  /// Returns a new [MultipleCollaboratorsFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MultipleCollaboratorsFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MultipleCollaboratorsFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MultipleCollaboratorsFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MultipleCollaboratorsFieldField(
  ...
  static List<MultipleCollaboratorsFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <MultipleCollaboratorsFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MultipleCollaboratorsFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MultipleCollaboratorsFieldField> mapFromJson(dynamic json) {
    final map = <String, MultipleCollaboratorsFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MultipleCollaboratorsFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of MultipleCollaboratorsFieldField-objects as value to a dart map
  static Map<String, List<MultipleCollaboratorsFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<MultipleCollaboratorsFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = MultipleCollaboratorsFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
