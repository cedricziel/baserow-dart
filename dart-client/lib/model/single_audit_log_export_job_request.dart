...
class SingleAuditLogExportJobRequest {
  /// Returns a new [SingleAuditLogExportJobRequest] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SingleAuditLogExportJobRequest &&
    other.url == url &&
  ...
  @override
  String toString() => 'SingleAuditLogExportJobRequest[url=$url, exportCharset=$exportCharset, csvColumnSeparator=$csvColumnSeparator, csvFirstRowHeader=$csvFirstRowHeader, filterUserId=$filterUserId, filterWorkspaceId=$filterWorkspaceId, filterActionType=$filterActionType, filterFromTimestamp=$filterFromTimestamp, filterToTimestamp=$filterToTimestamp, excludeColumns=$excludeColumns]';
  ...
  /// Returns a new [SingleAuditLogExportJobRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SingleAuditLogExportJobRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SingleAuditLogExportJobRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SingleAuditLogExportJobRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return SingleAuditLogExportJobRequest(
  ...
  static List<SingleAuditLogExportJobRequest> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SingleAuditLogExportJobRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SingleAuditLogExportJobRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SingleAuditLogExportJobRequest> mapFromJson(dynamic json) {
    final map = <String, SingleAuditLogExportJobRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SingleAuditLogExportJobRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SingleAuditLogExportJobRequest-objects as value to a dart map
  static Map<String, List<SingleAuditLogExportJobRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SingleAuditLogExportJobRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SingleAuditLogExportJobRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
