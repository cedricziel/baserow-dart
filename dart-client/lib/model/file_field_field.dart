...
class FileFieldField {
  /// Returns a new [FileFieldField] instance.
  FileFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FileFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'FileFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description]';
  ...
  /// Returns a new [FileFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FileFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FileFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FileFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return FileFieldField(
  ...
  static List<FileFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FileFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FileFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FileFieldField> mapFromJson(dynamic json) {
    final map = <String, FileFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FileFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of FileFieldField-objects as value to a dart map
  static Map<String, List<FileFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FileFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FileFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
