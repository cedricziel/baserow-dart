...
class UUIDFieldField {
  /// Returns a new [UUIDFieldField] instance.
  UUIDFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UUIDFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'UUIDFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description]';
  ...
  /// Returns a new [UUIDFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static UUIDFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UUIDFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UUIDFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return UUIDFieldField(
  ...
  static List<UUIDFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <UUIDFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UUIDFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, UUIDFieldField> mapFromJson(dynamic json) {
    final map = <String, UUIDFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UUIDFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of UUIDFieldField-objects as value to a dart map
  static Map<String, List<UUIDFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UUIDFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = UUIDFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
