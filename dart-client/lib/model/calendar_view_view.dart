...
class CalendarViewView {
  /// Returns a new [CalendarViewView] instance.
  CalendarViewView({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CalendarViewView &&
    other.id == id &&
  ...
  @override
  String toString() => 'CalendarViewView[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, table=$table, filterType=$filterType, filters=$filters, filterGroups=$filterGroups, sortings=$sortings, groupBys=$groupBys, decorations=$decorations, filtersDisabled=$filtersDisabled, publicViewHasPassword=$publicViewHasPassword, showLogo=$showLogo, ownershipType=$ownershipType, ownedById=$ownedById, dateField=$dateField, icalFeedUrl=$icalFeedUrl, icalPublic=$icalPublic, public=$public, slug=$slug]';
  ...
  /// Returns a new [CalendarViewView] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CalendarViewView? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CalendarViewView[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CalendarViewView[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CalendarViewView> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CalendarViewView>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CalendarViewView.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CalendarViewView> mapFromJson(dynamic json) {
    final map = <String, CalendarViewView>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CalendarViewView.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CalendarViewView-objects as value to a dart map
  static Map<String, List<CalendarViewView>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CalendarViewView>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CalendarViewView.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
