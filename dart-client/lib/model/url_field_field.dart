...
class URLFieldField {
  /// Returns a new [URLFieldField] instance.
  URLFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is URLFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'URLFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description]';
  ...
  /// Returns a new [URLFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static URLFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "URLFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "URLFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return URLFieldField(
  ...
  static List<URLFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <URLFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = URLFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, URLFieldField> mapFromJson(dynamic json) {
    final map = <String, URLFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = URLFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of URLFieldField-objects as value to a dart map
  static Map<String, List<URLFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<URLFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = URLFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
