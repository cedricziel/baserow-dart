...
class CountFieldUpdateField {
  /// Returns a new [CountFieldUpdateField] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CountFieldUpdateField &&
    other.name == name &&
    other.type == type &&
  ...
  @override
  String toString() => 'CountFieldUpdateField[name=$name, type=$type, description=$description, error=$error, dateShowTzinfo=$dateShowTzinfo, dateFormat=$dateFormat, numberSeparator=$numberSeparator, arrayFormulaType=$arrayFormulaType, durationFormat=$durationFormat, nullable=$nullable, dateTimeFormat=$dateTimeFormat, dateForceTimezone=$dateForceTimezone, numberDecimalPlaces=$numberDecimalPlaces, numberPrefix=$numberPrefix, dateIncludeTime=$dateIncludeTime, numberSuffix=$numberSuffix, throughFieldId=$throughFieldId, formulaType=$formulaType]';
  ...
  /// Returns a new [CountFieldUpdateField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CountFieldUpdateField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CountFieldUpdateField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CountFieldUpdateField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return CountFieldUpdateField(
  ...
  static List<CountFieldUpdateField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CountFieldUpdateField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CountFieldUpdateField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CountFieldUpdateField> mapFromJson(dynamic json) {
    final map = <String, CountFieldUpdateField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CountFieldUpdateField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CountFieldUpdateField-objects as value to a dart map
  static Map<String, List<CountFieldUpdateField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CountFieldUpdateField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CountFieldUpdateField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
