...
class ChoiceElementUpdateElement {
  /// Returns a new [ChoiceElementUpdateElement] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ChoiceElementUpdateElement &&
  ...
  @override
  String toString() => 'ChoiceElementUpdateElement[visibility=$visibility, styles=$styles, styleBorderTopColor=$styleBorderTopColor, styleBorderTopSize=$styleBorderTopSize, stylePaddingTop=$stylePaddingTop, styleMarginTop=$styleMarginTop, styleBorderBottomColor=$styleBorderBottomColor, styleBorderBottomSize=$styleBorderBottomSize, stylePaddingBottom=$stylePaddingBottom, styleMarginBottom=$styleMarginBottom, styleBorderLeftColor=$styleBorderLeftColor, styleBorderLeftSize=$styleBorderLeftSize, stylePaddingLeft=$stylePaddingLeft, styleMarginLeft=$styleMarginLeft, styleBorderRightColor=$styleBorderRightColor, styleBorderRightSize=$styleBorderRightSize, stylePaddingRight=$stylePaddingRight, styleMarginRight=$styleMarginRight, styleBackground=$styleBackground, styleBackgroundColor=$styleBackgroundColor, styleBackgroundFile=$styleBackgroundFile, styleBackgroundMode=$styleBackgroundMode, styleWidth=$styleWidth, roleType=$roleType, roles=$roles, label=$label, defaultValue=$defaultValue, required_=$required_, placeholder=$placeholder, options=$options, multiple=$multiple, showAsDropdown=$showAsDropdown, optionType=$optionType, formulaValue=$formulaValue, formulaName=$formulaName]';
  ...
  /// Returns a new [ChoiceElementUpdateElement] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ChoiceElementUpdateElement? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ChoiceElementUpdateElement[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ChoiceElementUpdateElement[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ChoiceElementUpdateElement(
        visibility: Visibility789Enum.fromJson(json[r'visibility']),
        styles: InputConfigBlock.fromJson(json[r'styles']),
  ...
  static List<ChoiceElementUpdateElement> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ChoiceElementUpdateElement>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ChoiceElementUpdateElement.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ChoiceElementUpdateElement> mapFromJson(dynamic json) {
    final map = <String, ChoiceElementUpdateElement>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ChoiceElementUpdateElement.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ChoiceElementUpdateElement-objects as value to a dart map
  static Map<String, List<ChoiceElementUpdateElement>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ChoiceElementUpdateElement>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ChoiceElementUpdateElement.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
