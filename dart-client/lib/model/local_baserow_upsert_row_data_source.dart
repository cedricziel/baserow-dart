...
class LocalBaserowUpsertRowDataSource {
  /// Returns a new [LocalBaserowUpsertRowDataSource] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LocalBaserowUpsertRowDataSource &&
    other.id == id &&
  ...
  @override
  String toString() => 'LocalBaserowUpsertRowDataSource[id=$id, integrationId=$integrationId, type=$type, schema=$schema, contextData=$contextData, contextDataSchema=$contextDataSchema, name=$name, pageId=$pageId, order=$order, tableId=$tableId, fieldMappings=$fieldMappings, rowId=$rowId]';
  ...
  /// Returns a new [LocalBaserowUpsertRowDataSource] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LocalBaserowUpsertRowDataSource? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LocalBaserowUpsertRowDataSource[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LocalBaserowUpsertRowDataSource[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LocalBaserowUpsertRowDataSource(
  ...
  static List<LocalBaserowUpsertRowDataSource> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LocalBaserowUpsertRowDataSource>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LocalBaserowUpsertRowDataSource.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LocalBaserowUpsertRowDataSource> mapFromJson(dynamic json) {
    final map = <String, LocalBaserowUpsertRowDataSource>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LocalBaserowUpsertRowDataSource.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LocalBaserowUpsertRowDataSource-objects as value to a dart map
  static Map<String, List<LocalBaserowUpsertRowDataSource>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LocalBaserowUpsertRowDataSource>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LocalBaserowUpsertRowDataSource.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
