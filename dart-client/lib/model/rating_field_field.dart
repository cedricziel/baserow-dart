...
class RatingFieldField {
  /// Returns a new [RatingFieldField] instance.
  RatingFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is RatingFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'RatingFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description, maxValue=$maxValue, color=$color, style=$style]';
  ...
  /// Returns a new [RatingFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static RatingFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "RatingFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "RatingFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return RatingFieldField(
  ...
  static List<RatingFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <RatingFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = RatingFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, RatingFieldField> mapFromJson(dynamic json) {
    final map = <String, RatingFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = RatingFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of RatingFieldField-objects as value to a dart map
  static Map<String, List<RatingFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<RatingFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = RatingFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
