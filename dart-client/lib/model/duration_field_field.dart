...
class DurationFieldField {
  /// Returns a new [DurationFieldField] instance.
  DurationFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is DurationFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'DurationFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description, durationFormat=$durationFormat]';
  ...
  /// Returns a new [DurationFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static DurationFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DurationFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DurationFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return DurationFieldField(
  ...
  static List<DurationFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <DurationFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = DurationFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, DurationFieldField> mapFromJson(dynamic json) {
    final map = <String, DurationFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DurationFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of DurationFieldField-objects as value to a dart map
  static Map<String, List<DurationFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DurationFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = DurationFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
