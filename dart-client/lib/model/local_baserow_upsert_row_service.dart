...
class LocalBaserowUpsertRowService {
  /// Returns a new [LocalBaserowUpsertRowService] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LocalBaserowUpsertRowService &&
    other.id == id &&
  ...
  @override
  String toString() => 'LocalBaserowUpsertRowService[id=$id, integrationId=$integrationId, type=$type, schema=$schema, contextData=$contextData, contextDataSchema=$contextDataSchema, tableId=$tableId, fieldMappings=$fieldMappings, rowId=$rowId]';
  ...
  /// Returns a new [LocalBaserowUpsertRowService] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LocalBaserowUpsertRowService? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LocalBaserowUpsertRowService[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LocalBaserowUpsertRowService[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LocalBaserowUpsertRowService(
  ...
  static List<LocalBaserowUpsertRowService> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LocalBaserowUpsertRowService>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LocalBaserowUpsertRowService.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LocalBaserowUpsertRowService> mapFromJson(dynamic json) {
    final map = <String, LocalBaserowUpsertRowService>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LocalBaserowUpsertRowService.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LocalBaserowUpsertRowService-objects as value to a dart map
  static Map<String, List<LocalBaserowUpsertRowService>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LocalBaserowUpsertRowService>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LocalBaserowUpsertRowService.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
