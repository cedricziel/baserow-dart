...
class RollupFieldField {
  /// Returns a new [RollupFieldField] instance.
  RollupFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is RollupFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'RollupFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description, error=$error, dateShowTzinfo=$dateShowTzinfo, dateFormat=$dateFormat, numberSeparator=$numberSeparator, arrayFormulaType=$arrayFormulaType, durationFormat=$durationFormat, nullable=$nullable, dateTimeFormat=$dateTimeFormat, dateForceTimezone=$dateForceTimezone, numberDecimalPlaces=$numberDecimalPlaces, numberPrefix=$numberPrefix, dateIncludeTime=$dateIncludeTime, numberSuffix=$numberSuffix, throughFieldId=$throughFieldId, targetFieldId=$targetFieldId, rollupFunction=$rollupFunction, formulaType=$formulaType]';
  ...
  /// Returns a new [RollupFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static RollupFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "RollupFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "RollupFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return RollupFieldField(
  ...
  static List<RollupFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <RollupFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = RollupFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, RollupFieldField> mapFromJson(dynamic json) {
    final map = <String, RollupFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = RollupFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of RollupFieldField-objects as value to a dart map
  static Map<String, List<RollupFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<RollupFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = RollupFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
