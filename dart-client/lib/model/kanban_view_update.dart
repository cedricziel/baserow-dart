...
class KanbanViewUpdate {
  /// Returns a new [KanbanViewUpdate] instance.
  KanbanViewUpdate({
    this.name,
    this.filterType,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is KanbanViewUpdate &&
    other.name == name &&
  ...
  @override
  String toString() => 'KanbanViewUpdate[name=$name, filterType=$filterType, filtersDisabled=$filtersDisabled, publicViewPassword=$publicViewPassword, ownershipType=$ownershipType, singleSelectField=$singleSelectField, cardCoverImageField=$cardCoverImageField, public=$public, slug=$slug]';
  ...
  /// Returns a new [KanbanViewUpdate] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static KanbanViewUpdate? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "KanbanViewUpdate[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "KanbanViewUpdate[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return KanbanViewUpdate(
  ...
  static List<KanbanViewUpdate> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <KanbanViewUpdate>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = KanbanViewUpdate.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, KanbanViewUpdate> mapFromJson(dynamic json) {
    final map = <String, KanbanViewUpdate>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = KanbanViewUpdate.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of KanbanViewUpdate-objects as value to a dart map
  static Map<String, List<KanbanViewUpdate>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<KanbanViewUpdate>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = KanbanViewUpdate.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
