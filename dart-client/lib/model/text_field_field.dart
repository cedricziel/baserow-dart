...
class TextFieldField {
  /// Returns a new [TextFieldField] instance.
  TextFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TextFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'TextFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description, textDefault=$textDefault]';
  ...
  /// Returns a new [TextFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static TextFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TextFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TextFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<TextFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TextFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TextFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TextFieldField> mapFromJson(dynamic json) {
    final map = <String, TextFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TextFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of TextFieldField-objects as value to a dart map
  static Map<String, List<TextFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TextFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TextFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
