...
class EmailFieldField {
  /// Returns a new [EmailFieldField] instance.
  EmailFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is EmailFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'EmailFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description]';
  ...
  /// Returns a new [EmailFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static EmailFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "EmailFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "EmailFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return EmailFieldField(
  ...
  static List<EmailFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EmailFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EmailFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EmailFieldField> mapFromJson(dynamic json) {
    final map = <String, EmailFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = EmailFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of EmailFieldField-objects as value to a dart map
  static Map<String, List<EmailFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EmailFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EmailFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
