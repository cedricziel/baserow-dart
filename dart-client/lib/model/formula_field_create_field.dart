...
class FormulaFieldCreateField {
  /// Returns a new [FormulaFieldCreateField] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FormulaFieldCreateField &&
    other.name == name &&
    other.type == type &&
  ...
  @override
  String toString() => 'FormulaFieldCreateField[name=$name, type=$type, description=$description, error=$error, dateShowTzinfo=$dateShowTzinfo, dateFormat=$dateFormat, numberSeparator=$numberSeparator, arrayFormulaType=$arrayFormulaType, durationFormat=$durationFormat, nullable=$nullable, dateTimeFormat=$dateTimeFormat, dateForceTimezone=$dateForceTimezone, numberDecimalPlaces=$numberDecimalPlaces, numberPrefix=$numberPrefix, dateIncludeTime=$dateIncludeTime, numberSuffix=$numberSuffix, formula=$formula, formulaType=$formulaType]';
  ...
  /// Returns a new [FormulaFieldCreateField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FormulaFieldCreateField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FormulaFieldCreateField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FormulaFieldCreateField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return FormulaFieldCreateField(
  ...
  static List<FormulaFieldCreateField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FormulaFieldCreateField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FormulaFieldCreateField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FormulaFieldCreateField> mapFromJson(dynamic json) {
    final map = <String, FormulaFieldCreateField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FormulaFieldCreateField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of FormulaFieldCreateField-objects as value to a dart map
  static Map<String, List<FormulaFieldCreateField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FormulaFieldCreateField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FormulaFieldCreateField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
