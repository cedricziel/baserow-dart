...
class TimelineViewView {
  /// Returns a new [TimelineViewView] instance.
  TimelineViewView({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TimelineViewView &&
    other.id == id &&
  ...
  @override
  String toString() => 'TimelineViewView[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, table=$table, filterType=$filterType, filters=$filters, filterGroups=$filterGroups, sortings=$sortings, groupBys=$groupBys, decorations=$decorations, filtersDisabled=$filtersDisabled, publicViewHasPassword=$publicViewHasPassword, showLogo=$showLogo, ownershipType=$ownershipType, ownedById=$ownedById, startDateField=$startDateField, endDateField=$endDateField, timescale=$timescale, public=$public, slug=$slug]';
  ...
  /// Returns a new [TimelineViewView] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static TimelineViewView? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TimelineViewView[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TimelineViewView[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<TimelineViewView> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TimelineViewView>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TimelineViewView.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TimelineViewView> mapFromJson(dynamic json) {
    final map = <String, TimelineViewView>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TimelineViewView.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of TimelineViewView-objects as value to a dart map
  static Map<String, List<TimelineViewView>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TimelineViewView>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TimelineViewView.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
