...
class LocalBaserowUserSourceUserSource {
  /// Returns a new [LocalBaserowUserSourceUserSource] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LocalBaserowUserSourceUserSource &&
    other.id == id &&
    other.uid == uid &&
  ...
  @override
  String toString() => 'LocalBaserowUserSourceUserSource[id=$id, uid=$uid, applicationId=$applicationId, integrationId=$integrationId, type=$type, name=$name, order=$order, authProviders=$authProviders, userCount=$userCount, userCountUpdatedAt=$userCountUpdatedAt, tableId=$tableId, emailFieldId=$emailFieldId, nameFieldId=$nameFieldId, roleFieldId=$roleFieldId]';
  ...
  /// Returns a new [LocalBaserowUserSourceUserSource] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LocalBaserowUserSourceUserSource? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LocalBaserowUserSourceUserSource[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LocalBaserowUserSourceUserSource[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LocalBaserowUserSourceUserSource(
  ...
  static List<LocalBaserowUserSourceUserSource> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LocalBaserowUserSourceUserSource>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LocalBaserowUserSourceUserSource.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LocalBaserowUserSourceUserSource> mapFromJson(dynamic json) {
    final map = <String, LocalBaserowUserSourceUserSource>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LocalBaserowUserSourceUserSource.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LocalBaserowUserSourceUserSource-objects as value to a dart map
  static Map<String, List<LocalBaserowUserSourceUserSource>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LocalBaserowUserSourceUserSource>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LocalBaserowUserSourceUserSource.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
