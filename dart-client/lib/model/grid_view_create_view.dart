...
class GridViewCreateView {
  /// Returns a new [GridViewCreateView] instance.
  GridViewCreateView({
    required this.name,
    required this.type,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is GridViewCreateView &&
    other.name == name &&
    other.type == type &&
  ...
  @override
  String toString() => 'GridViewCreateView[name=$name, type=$type, ownershipType=$ownershipType, filterType=$filterType, filtersDisabled=$filtersDisabled, rowIdentifierType=$rowIdentifierType, rowHeightSize=$rowHeightSize, public=$public, slug=$slug]';
  ...
  /// Returns a new [GridViewCreateView] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static GridViewCreateView? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GridViewCreateView[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GridViewCreateView[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return GridViewCreateView(
  ...
  static List<GridViewCreateView> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <GridViewCreateView>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = GridViewCreateView.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, GridViewCreateView> mapFromJson(dynamic json) {
    final map = <String, GridViewCreateView>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GridViewCreateView.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of GridViewCreateView-objects as value to a dart map
  static Map<String, List<GridViewCreateView>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<GridViewCreateView>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = GridViewCreateView.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
