...
class AuditLogExportJobJob {
  /// Returns a new [AuditLogExportJobJob] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AuditLogExportJobJob &&
    other.id == id &&
    other.type == type &&
  ...
  @override
  String toString() => 'AuditLogExportJobJob[id=$id, type=$type, progressPercentage=$progressPercentage, state=$state, humanReadableError=$humanReadableError, csvColumnSeparator=$csvColumnSeparator, csvFirstRowHeader=$csvFirstRowHeader, exportCharset=$exportCharset, filterUserId=$filterUserId, filterWorkspaceId=$filterWorkspaceId, filterActionType=$filterActionType, filterFromTimestamp=$filterFromTimestamp, filterToTimestamp=$filterToTimestamp, excludeColumns=$excludeColumns, createdOn=$createdOn, exportedFileName=$exportedFileName, url=$url]';
  ...
  /// Returns a new [AuditLogExportJobJob] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AuditLogExportJobJob? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AuditLogExportJobJob[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AuditLogExportJobJob[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return AuditLogExportJobJob(
  ...
  static List<AuditLogExportJobJob> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AuditLogExportJobJob>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AuditLogExportJobJob.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AuditLogExportJobJob> mapFromJson(dynamic json) {
    final map = <String, AuditLogExportJobJob>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AuditLogExportJobJob.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AuditLogExportJobJob-objects as value to a dart map
  static Map<String, List<AuditLogExportJobJob>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AuditLogExportJobJob>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AuditLogExportJobJob.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
