...
class IntegrationServiceDataSource {
  /// Returns a new [IntegrationServiceDataSource] instance.
  IntegrationServiceDataSource({
    required this.id,
    this.integrationId,
    required this.type,
    this.schema = const {},
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is IntegrationServiceDataSource &&
    other.id == id &&
  ...
  @override
  String toString() => 'IntegrationServiceDataSource[id=$id, integrationId=$integrationId, type=$type, schema=$schema, contextData=$contextData, contextDataSchema=$contextDataSchema, name=$name, pageId=$pageId, order=$order, tableId=$tableId, viewId=$viewId, filters=$filters, filterType=$filterType, searchQuery=$searchQuery, rowId=$rowId, sortings=$sortings, fieldId=$fieldId, aggregationType=$aggregationType, fieldMappings=$fieldMappings]';
  ...
  /// Returns a new [IntegrationServiceDataSource] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static IntegrationServiceDataSource? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "IntegrationServiceDataSource[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "IntegrationServiceDataSource[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return IntegrationServiceDataSource(
  ...
  static List<IntegrationServiceDataSource> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <IntegrationServiceDataSource>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = IntegrationServiceDataSource.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, IntegrationServiceDataSource> mapFromJson(dynamic json) {
    final map = <String, IntegrationServiceDataSource>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = IntegrationServiceDataSource.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of IntegrationServiceDataSource-objects as value to a dart map
  static Map<String, List<IntegrationServiceDataSource>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<IntegrationServiceDataSource>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = IntegrationServiceDataSource.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
