...
class RecordSelectorElementUpdateElement {
  /// Returns a new [RecordSelectorElementUpdateElement] instance.
  ...
  /// The text label for this record selector
  String label;

  /// This record selector default value.
  String defaultValue;

  /// The placeholder text which should be applied to the element.
  String placeholder;

  /// Whether this record selector allows users to choose multiple values.
  bool multiple;
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is RecordSelectorElementUpdateElement &&
  ...
  @override
  String toString() => 'RecordSelectorElementUpdateElement[visibility=$visibility, styles=$styles, styleBorderTopColor=$styleBorderTopColor, styleBorderTopSize=$styleBorderTopSize, stylePaddingTop=$stylePaddingTop, styleMarginTop=$styleMarginTop, styleBorderBottomColor=$styleBorderBottomColor, styleBorderBottomSize=$styleBorderBottomSize, stylePaddingBottom=$stylePaddingBottom, styleMarginBottom=$styleMarginBottom, styleBorderLeftColor=$styleBorderLeftColor, styleBorderLeftSize=$styleBorderLeftSize, stylePaddingLeft=$stylePaddingLeft, styleMarginLeft=$styleMarginLeft, styleBorderRightColor=$styleBorderRightColor, styleBorderRightSize=$styleBorderRightSize, stylePaddingRight=$stylePaddingRight, styleMarginRight=$styleMarginRight, styleBackground=$styleBackground, styleBackgroundColor=$styleBackgroundColor, styleBackgroundFile=$styleBackgroundFile, styleBackgroundMode=$styleBackgroundMode, styleWidth=$styleWidth, roleType=$roleType, roles=$roles, dataSourceId=$dataSourceId, itemsPerPage=$itemsPerPage, buttonLoadMoreLabel=$buttonLoadMoreLabel, propertyOptions=$propertyOptions, isPubliclySortable=$isPubliclySortable, isPubliclyFilterable=$isPubliclyFilterable, isPubliclySearchable=$isPubliclySearchable, required_=$required_, label=$label, defaultValue=$defaultValue, placeholder=$placeholder, multiple=$multiple, optionNameSuffix=$optionNameSuffix]';
  ...
  /// Returns a new [RecordSelectorElementUpdateElement] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static RecordSelectorElementUpdateElement? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "RecordSelectorElementUpdateElement[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "RecordSelectorElementUpdateElement[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return RecordSelectorElementUpdateElement(
  ...
  static List<RecordSelectorElementUpdateElement> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <RecordSelectorElementUpdateElement>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = RecordSelectorElementUpdateElement.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, RecordSelectorElementUpdateElement> mapFromJson(dynamic json) {
    final map = <String, RecordSelectorElementUpdateElement>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = RecordSelectorElementUpdateElement.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of RecordSelectorElementUpdateElement-objects as value to a dart map
  static Map<String, List<RecordSelectorElementUpdateElement>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<RecordSelectorElementUpdateElement>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = RecordSelectorElementUpdateElement.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
