...
class FieldField {
  /// Returns a new [FieldField] instance.
  FieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FieldField &&
    other.id == id &&
  ...
  /// Returns a new [FieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FieldField> mapFromJson(dynamic json) {
    final map = <String, FieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of FieldField-objects as value to a dart map
  static Map<String, List<FieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
