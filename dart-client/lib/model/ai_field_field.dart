...
class AIFieldField {
  /// Returns a new [AIFieldField] instance.
  AIFieldField({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AIFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'AIFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description, selectOptions=$selectOptions, aiGenerativeAiType=$aiGenerativeAiType, aiGenerativeAiModel=$aiGenerativeAiModel, aiOutputType=$aiOutputType, aiTemperature=$aiTemperature, aiPrompt=$aiPrompt, aiFileFieldId=$aiFileFieldId]';
  ...
  /// Returns a new [AIFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AIFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AIFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AIFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AIFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AIFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AIFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AIFieldField> mapFromJson(dynamic json) {
    final map = <String, AIFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AIFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AIFieldField-objects as value to a dart map
  static Map<String, List<AIFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AIFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AIFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
