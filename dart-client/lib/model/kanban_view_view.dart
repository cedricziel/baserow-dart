...
class KanbanViewView {
  /// Returns a new [KanbanViewView] instance.
  KanbanViewView({
    required this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is KanbanViewView &&
    other.id == id &&
  ...
  @override
  String toString() => 'KanbanViewView[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, table=$table, filterType=$filterType, filters=$filters, filterGroups=$filterGroups, sortings=$sortings, groupBys=$groupBys, decorations=$decorations, filtersDisabled=$filtersDisabled, publicViewHasPassword=$publicViewHasPassword, showLogo=$showLogo, ownershipType=$ownershipType, ownedById=$ownedById, singleSelectField=$singleSelectField, cardCoverImageField=$cardCoverImageField, public=$public, slug=$slug]';
  ...
  /// Returns a new [KanbanViewView] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static KanbanViewView? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "KanbanViewView[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "KanbanViewView[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return KanbanViewView(
  ...
  static List<KanbanViewView> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <KanbanViewView>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = KanbanViewView.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, KanbanViewView> mapFromJson(dynamic json) {
    final map = <String, KanbanViewView>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = KanbanViewView.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of KanbanViewView-objects as value to a dart map
  static Map<String, List<KanbanViewView>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<KanbanViewView>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = KanbanViewView.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
