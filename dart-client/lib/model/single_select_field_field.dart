...
class SingleSelectFieldField {
  /// Returns a new [SingleSelectFieldField] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SingleSelectFieldField &&
    other.id == id &&
  ...
  @override
  String toString() => 'SingleSelectFieldField[id=$id, tableId=$tableId, name=$name, order=$order, type=$type, primary=$primary, readOnly=$readOnly, immutableType=$immutableType, immutableProperties=$immutableProperties, description=$description, selectOptions=$selectOptions]';
  ...
  /// Returns a new [SingleSelectFieldField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SingleSelectFieldField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SingleSelectFieldField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SingleSelectFieldField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return SingleSelectFieldField(
  ...
  static List<SingleSelectFieldField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SingleSelectFieldField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SingleSelectFieldField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SingleSelectFieldField> mapFromJson(dynamic json) {
    final map = <String, SingleSelectFieldField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SingleSelectFieldField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SingleSelectFieldField-objects as value to a dart map
  static Map<String, List<SingleSelectFieldField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SingleSelectFieldField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SingleSelectFieldField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
