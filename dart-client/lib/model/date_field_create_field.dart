...
class DateFieldCreateField {
  /// Returns a new [DateFieldCreateField] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is DateFieldCreateField &&
    other.name == name &&
    other.type == type &&
  ...
  @override
  String toString() => 'DateFieldCreateField[name=$name, type=$type, description=$description, dateFormat=$dateFormat, dateIncludeTime=$dateIncludeTime, dateTimeFormat=$dateTimeFormat, dateShowTzinfo=$dateShowTzinfo, dateForceTimezone=$dateForceTimezone, dateForceTimezoneOffset=$dateForceTimezoneOffset]';
  ...
  /// Returns a new [DateFieldCreateField] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static DateFieldCreateField? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DateFieldCreateField[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DateFieldCreateField[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return DateFieldCreateField(
  ...
  static List<DateFieldCreateField> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <DateFieldCreateField>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = DateFieldCreateField.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, DateFieldCreateField> mapFromJson(dynamic json) {
    final map = <String, DateFieldCreateField>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DateFieldCreateField.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of DateFieldCreateField-objects as value to a dart map
  static Map<String, List<DateFieldCreateField>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DateFieldCreateField>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = DateFieldCreateField.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
